...
OUTPUT_PATH = os.getenv("AUDIO_OUTPUT_PATH", "recordings")
...
def transcribe_audio(audio_file, batch_size=16, compute_type="float16"):
    print(Fore.CYAN + f"Transcribing audio file: {audio_file}")
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(Fore.GREEN + f"Using {device.upper()} for transcription")
    
    try:
        # 1. Load the model
        model = whisperx.load_model("large-v2", device, compute_type=compute_type, download_root=MODEL_PATH)
        
        # 2. Transcribe with WhisperX (batched)
        audio = whisperx.load_audio(audio_file)
        result = model.transcribe(audio, batch_size=batch_size)
        
        # 3. Align WhisperX output
        model_a, metadata = whisperx.load_align_model(language_code=result["language"], device=device, model_dir=MODEL_PATH)
        result = whisperx.align(result["segments"], model_a, metadata, audio, device, return_char_alignments=False)
        
        # 4. Assign speaker labels
        diarize_model = whisperx.DiarizationPipeline(use_auth_token=HF_TOKEN, device=device, model_dir=MODEL_PATH)
        ...
        print(Fore.RED + f"An error occurred during transcription: {str(e)}")
        return None
...
def save_transcription(result, audio_file):
    ...
    with open(transcription_file, 'w', encoding='utf-8') as f:
        for segment in result['segments']:
            f.write(f"[{segment['start']:.2f}s - {segment['end']:.2f}s] Speaker {segment.get('speaker', 'UNKNOWN')}: {segment['text']}\n")
